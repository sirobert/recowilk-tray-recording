...
Status: deterministyczna maszyna stanów, szyfrowany DPAPI magazyn tokenów oraz testowane klienty Calendar/Meet zakończone; przepływ OAuth, integracja aplikacji i testy ręczne oczekują na implementację.
...
| 2026-08-08 | R-012, logika domenowa, tokeny i klienci REST | .NET 8, Windows DPAPI, podstawiony HTTP bez sieci i urządzeń | 7/7 testów automatyki, 3/3 tokenów, 5/5 Calendar/Meet; build Release 0 ostrzeżeń | Pełna brama: 90 testów przeszło, 1 test MF opt-in pominięty, 3 istniejące testy koordynatora niewykonalne w sandboxie z powodu zakazu zapisu do `%LocalAppData%`; wymagane OAuth i test manualny Meet |
